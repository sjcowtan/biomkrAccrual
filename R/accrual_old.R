...
#' Capping arm using structure info from trial_structure
#' @param class_list List consisting of one accrual object and 
#' one trial structure object
#' @param target_arm_size Maximum number of patients per arm
#' @param site_cap Maximum number of patients per site
#' @return Modified accrual object with capped week's accrual
#' 
cap_arm <- S7::new_generic("cap arm", "class_list")
S7::method(cap_arm, list(accrual, trial_structure)) <- 
  function(class_list, target_arm_size, site_cap) {
    # Name objects for easy reference
    accrual_obj <- class_list[[1]]
    structure_obj <- class_list[[2]]

    print(accrual_obj@accrual[accrual_obj@week, , ])
